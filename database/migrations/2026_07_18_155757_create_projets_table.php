...
return new class extends Migration
{
    public function up()
    {
        Schema::create('projets', function (Blueprint $table) {
            ...
            $table->enum('statutProjet', ['brouillon', 'soumis', 'en_examen', 'approuve', 'rejete', 'valide'])
                  ->default('brouillon');

            // NOTE : messageModification supprimée — c'était un doublon. Le message
            // de demande de modification est stocké dans commentaires (typeCommentaire
            // = 'demande'), avec les motifs sélectionnés via commentaire_motifs.

            // Étape 1 du workflow : approbation
            $table->timestamp('dateApprobation')->nullable();
            $table->foreignId('approbateur_id')->nullable()->constrained('users')->nullOnDelete();

            // Étape 2 du workflow : validation
            $table->timestamp('dateValidation')->nullable();
            $table->foreignId('validateur_id')->nullable()->constrained('users')->nullOnDelete();

            // Le motif de rejet n'est pas stocké ici : voir table commentaires
            // (projet->commentaires()->where('typeCommentaire', 'rejet')->latest())
